...
void CCustomObject::FillProp(LPCSTR pref, PropItemVec& items)
{
    ...
    V = PHelper().CreateAngle3	(items, PrepareKey(pref,"Transform\\Rotation"),	&PRotation,	-10000,	10000,0.01,3);
    V->OnChangeEvent.bind		(this,&CCustomObject::OnNumChangeRotation);
    V = PHelper().CreateVector	(items, PrepareKey(pref,"Transform\\Scale"),	&PScale, 	0.01,	10000,0.01,3);
